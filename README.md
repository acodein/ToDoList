# 📝 React To-Do List 

A minimal and stylish to-do list app built with React. It lets users add and remove tasks in a simple, intuitive interface with fun handwritten-style fonts and sticky-note-inspired visuals.

Built in codesandbox.(codesandbox link - https://fy8lvp.csb.app/)

## 🚀 Features 

- ➕ Add new tasks via button or pressing Enter
- ✅ Remove tasks by clicking on them
- 🎨 Handwritten and paper-inspired UI
- 🔁 Persistent state while the page is open (via React state hooks)



## 💻 Getting Started

### Prerequisites

- Node.js and npm installed on your machine
  
### Installation

```bash
git clone https://github.com/yourusername/react-todo-list.git
cd react-todo-list
npm install

🙌 Acknowledgments
UI inspired by sticky notes & journaling themes ✏️

Font from Google Fonts

